...
class GSync extends SeedObject
{
	...
	public static $TStatus = array(
		self::STATUS_CANCELED => 'GSyncStatusShortCanceled'
		,self::STATUS_DRAFT => 'GSyncStatusShortDraft'
		,self::STATUS_VALIDATED => 'GSyncStatusShortValidated'
//		,self::STATUS_REFUSED => 'GSyncStatusShortRefused'
//		,self::STATUS_ACCEPTED => 'GSyncStatusShortAccepted'
	);
...
//        ),
    ...
    private function getNextRef()
    {
		...
		$mask = !empty($conf->global->GSYNC_REF_MASK) ? $conf->global->GSYNC_REF_MASK : 'MM{yy}{mm}-{0000}';
		$ref = get_next_value($db, $mask, 'gsync', 'ref');
...
//            $this->fk_user_valid = $user->id;
    ...
    public function getNomUrl($withpicto = 0, $moreparams = '')
    {
        ...
        $result='';
        $label = '<u>' . $langs->trans("ShowGSync") . '</u>';
        if (! empty($this->ref)) $label.= '<br><b>'.$langs->trans('Ref').':</b> '.$this->ref;

        $linkclose = '" title="'.dol_escape_htmltag($label, 1).'" class="classfortooltip">';
        $link = '<a href="'.dol_buildpath('/gsync/card.php', 1).'?id='.$this->id.urlencode($moreparams).$linkclose;
...
//        $picto='gsync@gsync';
    ...
    public static function LibStatut($status, $mode)
    {
        ...
        if ($status==self::STATUS_CANCELED) { $statusType='status9'; $statusLabel=$langs->trans('GSyncStatusCancel'); $statusLabelShort=$langs->trans('GSyncStatusShortCancel'); }
        elseif ($status==self::STATUS_DRAFT) { $statusType='status0'; $statusLabel=$langs->trans('GSyncStatusDraft'); $statusLabelShort=$langs->trans('GSyncStatusShortDraft'); }
        elseif ($status==self::STATUS_VALIDATED) { $statusType='status1'; $statusLabel=$langs->trans('GSyncStatusValidated'); $statusLabelShort=$langs->trans('GSyncStatusShortValidate'); }
        elseif ($status==self::STATUS_REFUSED) { $statusType='status5'; $statusLabel=$langs->trans('GSyncStatusRefused'); $statusLabelShort=$langs->trans('GSyncStatusShortRefused'); }
        elseif ($status==self::STATUS_ACCEPTED) { $statusType='status6'; $statusLabel=$langs->trans('GSyncStatusAccepted'); $statusLabelShort=$langs->trans('GSyncStatusShortAccepted'); }
