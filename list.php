...
$hookmanager->initHooks(array('gsynclist'));
...
llxHeader('', $langs->trans('GSyncList'), '', '');

//$type = GETPOST('type');
//if (empty($user->rights->gsync->all->read)) $type = 'mine';
...
$sql.= ' FROM '.MAIN_DB_PREFIX.'gsync t ';

if (!empty($object->isextrafieldmanaged))
{
    $sql.= ' LEFT JOIN '.MAIN_DB_PREFIX.'gsync_extrafields et ON (et.fk_object = t.rowid)';
}

$sql.= ' WHERE 1=1';
//$sql.= ' AND t.entity IN ('.getEntity('GSync', 1).')';
...
$formcore = new TFormCore($_SERVER['PHP_SELF'], 'form_list_gsync', 'GET');
...
$listViewConfig = array(
	...
	,'list' => array(
		'title' => $langs->trans('GSyncList')
		,'image' => 'title_generic.png'
		,'picto_precedent' => '<'
		,'picto_suivant' => '>'
		,'noheader' => 0
		,'messageNothing' => $langs->trans('NoGSync')
	...
	,'search' => array(
		...
		,'status' => array('search_type' => GSync::$TStatus, 'to_translate' => true) // select html, la clé = le status de l'objet, 'to_translate' à true si nécessaire
	)
	,'translate' => array()
	,'hide' => array(
